...
import { Mail, Lock, AlertCircle, RefreshCw, UserCircle2, ArrowRight, Eye, EyeOff, Feather, Sun, Moon } from 'lucide-react';
    ...
    <div className="bg-brand-cream dark:bg-aether-dark text-slate-800 dark:text-aether-on-surface min-h-screen flex items-center justify-center p-6 overflow-hidden relative select-none transition-colors duration-300">
      ...
      <main className="relative z-10 w-full max-w-[440px] flex items-center justify-center min-h-screen py-10">
        ...
        >
          ...
          <form onSubmit={handleSubmit} className="w-full flex flex-col gap-5">
            ...
            <div className="flex flex-col gap-1.5">
              <div className="flex justify-between items-center px-1">
                <label className="text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-aether-on-surface-variant">
                  Password
                </label>
                <a className="text-xs font-bold text-emerald-600 dark:text-aether-secondary hover:text-emerald-500 dark:hover:text-aether-primary transition-colors" href="#">
                  Forgot Password?
                </a>
              </div>
              <div className="relative">
            ...
            <label className="flex items-center gap-2 cursor-pointer group w-fit">
              <div className="relative flex items-center">
                <input 
                  type="checkbox"
                  className="peer appearance-none w-4 h-4 rounded border border-slate-300 dark:border-aether-outline-variant bg-white dark:bg-aether-container-low checked:bg-emerald-600 dark:checked:bg-aether-primary checked:border-emerald-600 dark:checked:border-aether-primary transition-all cursor-pointer"
                />
                <span className="absolute inset-0 text-white dark:text-aether-container-lowest text-[12px] flex items-center justify-center font-bold opacity-0 peer-checked:opacity-100 transition-opacity pointer-events-none">✓</span>
              </div>
              <span className="text-xs text-slate-500 dark:text-aether-on-surface-variant group-hover:text-slate-800 dark:group-hover:text-white transition-colors">
          ...
          <div className="w-full flex flex-col items-center gap-4 pt-4 border-t border-slate-100 dark:border-white/5">
            <p className="text-xs text-slate-500 dark:text-aether-on-surface-variant">
              ...
              <Link to="/signup" className="text-emerald-600 dark:text-aether-secondary font-bold hover:underline transition-all">
                Sign Up
              </Link>
            </p>
        ...
        <div className="absolute bottom-8 left-1/2 -translate-x-1/2 text-center w-full">
          <p className="text-[10px] text-slate-450 dark:text-aether-outline-variant/60 flex items-center justify-center gap-1.5 uppercase tracking-wider font-semibold">
